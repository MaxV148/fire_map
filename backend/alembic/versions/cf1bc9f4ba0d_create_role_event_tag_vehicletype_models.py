...
def upgrade() -> None:
    ...
    sa.ForeignKeyConstraint(['vehicle_id'], ['vehicletype.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index('idx_event_location', 'event', ['location'], unique=False, postgresql_using='gist',if_not_exists=True)
    ...
    op.drop_index('idx_exercises_location', table_name='exercises', postgresql_using='gist')
...
def downgrade() -> None:
    ...
    sa.Column('id', sa.INTEGER(), autoincrement=True, nullable=False),
    sa.Column('location', geoalchemy2.types.Geometry(geometry_type='POINT', from_text='ST_GeomFromEWKT', name='geometry', _spatial_index_reflected=True), autoincrement=False, nullable=True),
    ...
    sa.ForeignKeyConstraint(['trainer'], ['user.id'], name='exercises_trainer_fkey'),
    sa.PrimaryKeyConstraint('id', name='exercises_pkey')
    )
    op.create_index('ix_exercises_id', 'exercises', ['id'], unique=False)
    op.create_index('idx_exercises_location', 'exercises', ['location'], unique=False, postgresql_using='gist')
    ...
    op.drop_index('idx_event_location', table_name='event', postgresql_using='gist',if_exists=True)
    op.drop_table('event')
